...
def test_admin_email_filter(client, admin_token, many_users):
    res = client.get("api/v1/admin/users?page=1&limit=10&email=email5@example.com", headers={
...
def test_admin_username_sort(client, admin_token, many_users):
    res = client.get("api/v1/admin/users?page=1&limit=10&sort=username", headers={
        "Authorization": f"Bearer {admin_token}"
    })
    data = res.get_json()
